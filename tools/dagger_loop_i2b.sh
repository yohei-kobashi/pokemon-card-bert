...
#
# BATCH 32 x ACCUM 1, not 8 x 4. Same 32 rows per optimiser step, so the optimisation trajectory
# is unchanged and only the speed differs -- measured on an A100 over five configs at a constant
# 32 rows/step:
#
#     bsz  8 accum 4   2.02 s/it   15.84 rows/s   15.7 GiB
#     bsz 16 accum 2   1.86 s/it   17.20 rows/s   18.9 GiB
#     bsz 32 accum 1   1.83 s/it   17.49 rows/s   17.5 GiB     +10.4%
#
# and 1.75 s/it (+15.5%) with --no-grad-ckpt, which is kept OFF here for a reason: at bsz 32 that
# flag moved peak VRAM by only 0.7 GiB, where truly disabling activation checkpointing on a 4B
# model must move it by tens of GiB. The flag is not reaching the model, so its 4% is unexplained
# and not banked. bsz 64 was faster still (19.63 rows/s) but doubles rows per optimiser step,
# which is a different training run, not a speed knob.
#

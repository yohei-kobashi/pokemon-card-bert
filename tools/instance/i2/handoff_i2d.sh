#!/bin/bash
# Swap dagger_loop_i2c.sh for dagger_loop_i2d.sh (seeded screen + anchored collection) at the round-3 boundary.
#
# Not an in-place edit: bash reads a running script by BYTE OFFSET, so editing i2c while it
...
STATE=/root/loop_i2
LOG=$STATE/loop.log
HLOG=/root/handoff_i2d.log
LOOP_PID=${LOOP_PID:?set LOOP_PID to the running i2c pid}
DONE_ROUND=${DONE_ROUND:-3}
...
say "waiting for 'round $DONE_ROUND done' in $LOG (i2c pid $LOOP_PID, giving it ${MAX_WAIT_H}h)"
T0=$(date +%s)
while :; do
  ...
  if ! kill -0 "$LOOP_PID" 2>/dev/null; then
    say "ABORT: i2c (pid $LOOP_PID) exited without finishing round $DONE_ROUND -- last lines:"
    tr '\r' '\n' < "$LOG" | grep -av "^$" | tail -6
    exit 1
  fi
...
sleep 5
if pgrep -f "dagger_loop_i2c.sh" > /dev/null; then
  say "i2c did not stop on SIGTERM -- sending SIGKILL"
  pkill -9 -f "dagger_loop_i2c.sh" 2>/dev/null
  sleep 3
fi
say "stopped i2c; remaining loop/scorer processes:"
...
MODEL=$NEXT_MODEL START_ROUND=$NEXT_ROUND DEADLINE_H=${DEADLINE_H:-96} \
  setsid nohup bash tools/dagger_loop_i2d.sh < /dev/null > /root/i2d_boot.log 2>&1 &
sleep 5
say "started i2d from $NEXT_MODEL at round $NEXT_ROUND:"
ps -eo pid,etime,cmd | grep -a "[d]agger_loop_i2d.sh"
tail -4 "$LOG"

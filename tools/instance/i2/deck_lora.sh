...
#
#   bash /root/deck_lora.sh marnie_grimmsnarl 1
set -u
...
# ---------------------------------------------------------------- 1. collect
if ! ls ${TR_GLOB}0.jsonl.gz >/dev/null 2>&1; then
    gpu_wait
...
if [ ! -s "$PAIRS" ]; then
    echo "$TAG|$DECK|$BUDGET|$PLAYOUTS" > /root/branch_request2
    say "branch requested from instance1 (tag $TAG, budget $BUDGET, playouts $PLAYOUTS)"
    ...
    if [ ! -s "$PAIRS" ]; then
        ...
        RL_PRIZE_GAMMA=0.25 CUDA_VISIBLE_DEVICES= PYTHONPATH=cg-lib nice -n 5 \
            ...
            --only-deck "$DECK" --budget 4000 --per-game 15 --margin-min 0.01 \
...
grep -aq "PROBE OK" "$STATE/probe_$TAG.log" || { say "PROBE FAILED"; tail -4 "$STATE/probe_$TAG.log"; exit 1; }

...
MODEL=qwen:/root/out/dpo_r1                 # adopted on the user's criterion: 52.0% on the 11
OPPS=$(python3 -c "
...
print(','.join(d[$SH::3]))")
  ...
  PYTHONPATH=cg-lib nohup python3 tools/lm_mirror_log.py --model "$MODEL" \
    --protagonist dragapult_dusknoir --decks "$DK" --games 150 --seed $((40000 + SH * 1000)) \
    --out /root/lmlog_r4.s$SH.jsonl.gz --trace-out /root/traces_r4.s$SH.jsonl.gz \
    --mirror-so "$SO" > /root/collect_r4.s$SH.log 2>&1 &
  j=$((j+1))
done
say "launched $j shards"
wait
grep -ahE "^  [a-z_]+ +[0-9]+ games" /root/collect_r4.s*.log | tail -12
python3 -c "
import gzip, json, glob, collections
n=0; c=collections.Counter()
for f in sorted(glob.glob('/root/traces_r4.s*.jsonl.gz')):
    for line in gzip.open(f,'rt'):
        d=json.loads(line)
        if d.get('header'): continue
        n+=1; c[(d['deck0'],d['deck1'])]+=1
print('[c4] %d games over %d matchup orderings' % (n, len(c)))
"
say COLLECT_R4_DONE

...
PAIRS=/root/dpo_r5.jsonl.gz
FROM=/root/out/dpo_r4                 # adopted: 53.9% on the 11 decks, +3.80pt vs i2_r7
...
say() { echo "[dpo5 $(date -u +%m-%d_%H:%M:%S)] $*"; }
...
python3 tools/instance/dpo_teacher.py --data "$PAIRS" --init-from "$FROM" \
  --card-first "$VOCAB" --out /root/out/dpo_probe5 --probe --lr 5e-5 > "$STATE/probe5.log" 2>&1
grep -aq "PROBE OK" "$STATE/probe5.log" || { say "PROBE FAILED"; tail -3 "$STATE/probe4.log"; exit 1; }
...
python3 tools/instance/dpo_teacher.py --data "$PAIRS" --init-from "$FROM" --ref-from "$REF" \
  ...
  > "$STATE/train5.log" 2>&1 || { say "train FAILED"; tail -6 "$STATE/train4.log"; exit 1; }
grep -aE "\[ref\]|\[cdpo\]|FINAL|saved" "$STATE/train5.log" | tail -6
...
while read -r DK; do
  ...
  PYTHONPATH=cg-lib nohup python3 tools/mirror_match.py $DK --a engine --b "qwen:$OUT" \
      ...
      --out "$STATE/gate_dpo5.$j.json" > "$STATE/gate_dpo5.$j.log" 2>&1 &
  j=$((j+1))
...
def load(p, n=3):
    ...
    return d
cur = load("gate_dpo5"); r1 = load("gate_dpo4")
...
paired("dpo_r4 (the adopted checkpoint)", r1)

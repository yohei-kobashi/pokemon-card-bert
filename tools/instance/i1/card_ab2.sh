...
O=${O:-/root/loop_dusk/card_ab2}; mkdir -p $O
GAMES=${GAMES:-600}
OPPS=${OPPS:-marnie_grimmsnarl,alakazam_nz,ogerpon_mono,dudunsparce_box,slowking}
say() { echo "[cardab $(date -u +%m-%d_%H:%M:%S)] $*"; }

for D in ${DECKS:-dragapult_dusknoir dragapult_dusknoir_tower}; do
...
python3 - "$O/dragapult_dusknoir.json" "$O/dragapult_dusknoir_tower.json" <<'PY'
...
print("\n  %-22s %10s %10s %9s" % ("opponent", "current", "Crisp3/Tower2", "delta"))
ta = tb = na = nb = 0
...
if na and nb:
    ...
    print("\nVERDICT:", "swap in the 2nd Watchtower" if d > 2 * 100 * se
          else "no measurable difference -- keep 4 Crispin")
PY
say "CARD_AB_DONE"

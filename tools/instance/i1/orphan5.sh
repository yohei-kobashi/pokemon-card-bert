...
until grep -aq RESTART5_DONE /root/restart5.log 2>/dev/null; do sleep 20; done
sleep 30
for P in $(pgrep -f 'setup_search,front_dive,promote_dive,promote_line:hf'); do
    if ! tr '\0' ' ' < /proc/$P/cmdline | grep -q lethal_boss; then
        kill "$P" 2>/dev/null && echo "[orphan4] killed $P" >> /root/restart5.log
    fi
done
echo '[orphan4] done' >> /root/restart5.log

...
set -u
say() { echo "[br4 $(date -u +%m-%d_%H:%M:%S)] $*"; }
...
ls /root/traces_r4.s*.jsonl.gz >/dev/null 2>&1 || { say "no traces staged"; exit 1; }
say "pulled $(ls /root/traces_r4.s*.jsonl.gz | wc -l) trace shards"
CUDA_VISIBLE_DEVICES= PYTHONPATH=cg-lib nice -n 10 python3 tools/dpo_branch.py \
  --traces "$(ls /root/traces_r4.s*.jsonl.gz | paste -sd,)" \
  --budget 20000 --per-game 15 --margin-min 0.01 --playouts 32 --workers 16 \
  --out /root/dpo_r4.jsonl.gz || { say "branch FAILED"; exit 1; }
python3 -c "
import gzip, json, collections, statistics as st
rs=[json.loads(l) for l in gzip.open('/root/dpo_r4.jsonl.gz','rt')]
...
print('[br4] %d pairs | seat %s | dQ p50 %.3f' % (len(rs), dict(c), dq[len(dq)//2]))
print('[br4] piloting: %s' % dict(p.most_common(4)))
print('[br4] top opponents: %s' % dict(o.most_common(5)))
"
N=$(zcat /root/dpo_r4.jsonl.gz | wc -l)
scp -q $I2 -P 19839 /root/dpo_r4.jsonl.gz root@175.155.64.145:/root/dpo_r4.jsonl.gz \
  && say "instance2 has dpo_r4 ($N pairs)" || { say "SHIP FAILED"; exit 1; }
say BRANCH_R4_DONE

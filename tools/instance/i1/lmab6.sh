...
set -u
LOG=/root/lmab6.log
say() { echo "[lmab6 $(date -u +%m-%d_%H:%M:%S)] $*" >> "$LOG"; }
grep -aq LMAB6_DONE "$LOG" 2>/dev/null && exit 0
...
G=${G:-150}
DEADLINE=$(date -u -d 2026-08-16T09:30:00Z +%s)
...
PYTHONPATH=cg-lib python3 -u tools/gate_protagonist.py --deck dragapult_dusknoir --opp "$OPPS" \
    ...
    --arm "tips=planfilter:$B,$F,candy_line,noir_critical,stadium_bump:$CKPT@dusk" \
    --arm "spike=planfilter:$B,$F,spike_candy,spike_race:$CKPT@dusk" \
    --arm "all=planfilter:$B,$F,candy_line,noir_critical,stadium_bump,spike_candy,spike_race:$CKPT@dusk" \
    --games "$G" --seed 1 --baseline base --out /root/lmab6.json > /root/lmab6_run.log 2>&1
RC=$?

{
    echo "=============================================================="
    echo " PRO-TIPS RULES (candy_line/noir_critical/stadium_bump + spike) ON THE SHIPPING PILOT -- $(date -u +%F\ %H:%M) UTC"
    ...
    echo "=============================================================="
    grep "  dragapult_dusknoir " /root/lmab6_run.log
    echo
    sed -n "/^arm /,\$p" /root/lmab6_run.log
    echo
    echo "context: pokehubguide says Ogerpon is THE unfavorable matchup: race with spikes."
} > /root/lmab6_report.txt 2>&1

say "gate exited rc=$RC; report in /root/lmab6_report.txt"
say LMAB6_DONE

...
for t in sorted(want_tokens):
    ...
    if tid is not None and tid != unk:
        ...
        missing.append(t)
json.dump(sorted(keep), open('/root/onnx_sub3/keep_ids.json', 'w'))
...
say "building bundle dusk_v3"
python3 tools/build_rerank_submission.py dragapult_dusknoir \
    --onnx "$W/pruned/model_wonly_int8.onnx" \
    --tokenizer "$W/pruned/model" \
    ${REMAP:+--remap "$REMAP"} \
    --wrap "$WRAP" \
    --pfmt dusk --tag dusk_v3 \
    --threads 4 --max-len 512 --time-budget 480 \
    --out /root/subm > /root/subm_dusk_v3.log 2>&1 \
    || { say "BUILD FAILED"; tail -30 /root/subm_dusk_v3.log; exit 1; }
grep -aE "SELFCHECK|selfcheck|MiB|cap|wrap" /root/subm_dusk_v3.log | tail -10
ls -la /root/subm/dusk_v3.tar.gz
say "SUB3_BUILD_DONE champion=$CKPT"

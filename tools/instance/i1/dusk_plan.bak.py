...
def opportunities(obs, seat=None):
    ...
    clops_fire = (_scope(lambda i, o, t: t.startswith("ability:c%d" % DUSCLOPS))
                  if _CLOPS_HOLD else set())
    if clops_fire and DUSKNOIR in hand_ids and DUSCLOPS in my_ids and len(opts) > len(clops_fire):
        out["clops_hold"] = (set(range(len(opts))) - clops_fire, set(range(len(opts))))
    ...
    if _NEW_EXCL:
        ...
        _prize_left = len(me.get("prize") or [])
        _oa = (opp.get("active") or [None])[0]
        if isinstance(_oa, dict) and _prize_left:
            _def = _CD3.get(_oa.get("id"))
            _take = _prizes_for(_def)
            # Phantom Dive is TWO knock-outs in one attack when the board lines up: 200 to the
            # Active AND 6 counters that can finish damaged bench bodies. The first version only
            # asked "does knocking out the Active close the game", so a 2-left board that needed
            # Active(1) + a 60 HP bench body(1) never fired. The bench half is a tiny knapsack
            # -- at most 5 bench bodies, ceil(hp/10) counters each, 6 counters total -- so brute
            # force over subsets is exact and costs nothing.
            _bench_best = 0
            _bitems = []
            for b in (opp.get("bench") or []):
                if isinstance(b, dict) and 0 < (b.get("hp") or 0) <= 60:
                    _bitems.append(((b["hp"] + 9) // 10, _prizes_for(_CD3.get(b.get("id")))))
            if _bitems:
                import itertools as _it
                for r in range(1, len(_bitems) + 1):
                    for combo in _it.combinations(_bitems, r):
                        if sum(c for c, _v in combo) <= 6:
                            _bench_best = max(_bench_best, sum(v for _c, v in combo))
            if _take + _bench_best >= _prize_left:
                _mine_act = (me.get("active") or [None])[0]
                _att = _CD3.get((_mine_act or {}).get("id")) if isinstance(_mine_act, dict) else None
                _hp = _oa.get("hp") or 0
                kill = set()
                for i, o in enumerate(opts):
                    if not isinstance(o, dict) or not o.get("attackId"):
                        continue
                    d = _attack_damage(o["attackId"])
                    if not d:
                        continue
                    # weakness doubles, resistance subtracts 30 -- the standard modifiers, and
                    # both are read off the DEFENDER's card rather than assumed
                    if _att is not None and _def is not None:
                        if getattr(_def, "weakness", None) == getattr(_att, "energyType", None):
                            ...
                            d -= 30
                    _active_take = _take if d >= _hp > 0 else 0
                    _bench_take = _bench_best if o.get("attackId") == PHANTOM_DIVE else 0
                    # the attack is lethal if what it takes -- Active, bench, or both -- covers
                    # our remaining prizes; either half alone can carry it
                    if (_active_take or _bench_take) and _active_take + _bench_take >= _prize_left:
                        kill.add(i)

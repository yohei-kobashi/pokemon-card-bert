...
#
# Training starts FROM BASE by default on the UNION of every DAgger file collected so far, so a
# round that comes out worse cannot compound. Note that this loses nothing: the accumulated data
# carries the progress, not the weights.
#
# What continuing DOES buy is time -- 4.3 h of a 7.2 h round -- and more rounds mean more DAgger
# coverage. So the loop EARNS the right to continue: after two consecutive rounds that improve
# the median by more than +2 pp it switches to continuing from the previous model with a shorter
# run, and it reverts to base the moment a round regresses. The cost of continuing, beyond
# compounding, is that the model becomes a function of the round ORDER rather than of the data,
# which makes any regression much harder to attribute -- so it is not the default.
#
# Collection always uses the NEWEST model: the point is to fix the errors the current policy makes.
#
# Env:
#   KIND            rerank | qwen
...
PY
  ...
  # earn-the-right-to-continue: two straight improvements over +2 pp
  CONT=$(python3 - "$STATE/history.tsv" <<'PY'
import sys
try:
    rows = [l.split("\t") for l in open(sys.argv[1]).read().strip().split("\n") if l.strip()]
except Exception:
    rows = []
med = [float(r[2]) for r in rows]
print(1 if len(med) >= 3 and med[-1] - med[-2] > 0.02 and med[-2] - med[-3] > 0.02 else 0)
PY
)
  [ "$CONT" = 1 ] && say "trend improving twice over +2pp -> CONTINUING from $MODEL (shorter run)" \
                 || say "training from BASE (no sustained improvement, or too early to tell)"
...
")
  ...
  if [ "$KIND" = qwen ]; then
    ...
    # run, and add_domain_tokens would otherwise re-randomise the 2,971 rows it just learned.
...
PY
    ...
    if [ "$CONT" = 1 ]; then
      ...
      FROM=(--deadline-h 5 --max-samples 600000 --lr 2e-5)
    fi
    python3 tools/train_rerank.py --data "$MIX" --out "$OUT" "${FROM[@]}" \
        --pair-batch 32 --accum 12 --max-len 768 \

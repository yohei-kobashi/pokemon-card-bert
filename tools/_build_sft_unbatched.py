...
def _multipick_pairs(step):
    ...
    for pos_i in order:
        ...
        pairs.append((serialize_stateless(sub), encode_option(opts[pos_i], obs)))
        picked.append(pos_i)
    if lo <= len(picked) < hi:                 # heuristic chose to stop early
        sub, _remaining, allow_stop = multipick_substate(obs, picked)
        if allow_stop:
            pairs.append((serialize_stateless(sub), STOP))
    return pairs
...
def _prefetch_scores(steps, by_i, adopt):
    ...
    return None  # A/B: force the old per-candidate pair() path
            ...
            for header, steps in _read_game(path):
                ...
                for s in steps:
                    ...
                    if is_main:
                        ...
                        pairs = [(serialize_stateless(s["obs"]), action)]
                    else:                                      # sub-selection
                        ...
                        if lo == 1 and hi == 1:                # exactly one -> single sample
                            pairs = [(serialize_stateless(s["obs"]), _executed_target(s))]
                        else:                                  # multi / optional -> sequential
                            pairs = _multipick_pairs(s)

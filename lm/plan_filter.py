...
PROHIBITIONS = frozenset({
    ...
    "duskull_shelter",   # do not bench Duskull into Boss range while Ogerpon farms the bench
    "noir_critical",     # do not spend Dusknoir's one blast on less than two prizes
    "hammer_spare",      # do not Hammer the body this turn's damage already kills
    "draw_cap",          # mulligan bonus draw: take at most one card
})
...
def make_plan_rule(lm_agent, rules, strict=True, decider=None):
    ...
    def f(obs):
        ...
        _upto1 = os.environ.get("PLAN_UPTO1", "1") not in ("", "0")
        if strict:
